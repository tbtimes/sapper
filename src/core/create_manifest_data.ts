...
export default function create_manifest_data(
  ...
  if (!fs.existsSync(cwd)) {
    throw new Error(
      `As of Sapper 0.21, the routes/ directory should become src/routes/`
    );
  }
    ...
    if (/preload/.test(source)) {
      try {
        const { vars } = svelte.compile(
          source.replace(/<style\b[^>]*>[^]*?<\/style>/g, ""),
          { generate: false }
        );
        return vars.some(
          (variable: any) =>
            variable.module && variable.export_name === "preload"
        );
      } catch (err) {}
    }
  ...
  function walk(
    ...
    const items = fs
      ...
      .map(basename => {
        ...
        parts.forEach(part => {
          ...
          if (
            part.qualifier &&
            /[\(\)\?\:]/.test(part.qualifier.slice(1, -1))
          ) {
            throw new Error(
              `Invalid route ${file} — cannot use (, ), ? or : in route qualifiers`
            );
          }
        });
    ...
    items.forEach(item => {
      ...
      if (item.is_dir) {
        const component = find_layout(
          "_layout",
          `${get_slug(item.file)}__layout`,
          item.file
        );
...
const optional_spread_pattern = spread_pattern + "?";
const optional_spread_with_leading_slash_pattern = "(?:\\/" + spread_pattern + ")?";
